...
class PPO:
    def __init__(
        ...
        self.actor = Actor(log_level=20, log_path=log_path, device=self.device).to(
            self.device
        )
        self.critic = Critic(log_level=20, log_path=log_path, device=self.device).to(
            self.device
        )
    ...
    def _get_gaes(
        ...
        return torch.FloatTensor(gaes).to(self.device), torch.FloatTensor(target).to(
            self.device
        )
    ...
    def train(
        ...
        for agent in states.keys():
            ...
            for s, ns in zip(states[agent], next_states[agent]):
                input_states.append(
                    [
                        torch.FloatTensor(s["world-map"]).unsqueeze(0).to(self.device),
                        torch.FloatTensor(s["flat"]).unsqueeze(0).to(self.device),
                    ]
                )
                input_next_states.append(
                    [
                        torch.FloatTensor(ns["world-map"]).unsqueeze(0).to(self.device),
                        torch.FloatTensor(ns["flat"]).unsqueeze(0).to(self.device),
                    ]
                )
            ...
            values = torch.FloatTensor([torch.squeeze(v) for v in values]).to(
                self.device
            )
            next_values = torch.FloatTensor([torch.squeeze(v) for v in next_values]).to(
                self.device
            )
            # Log
            self.logger.debug(f"Values: {[round(float(v), 3) for v in values]}")
            self.logger.debug(
                f"Next values: {[round(float(v), 3) for v in next_values]}"
            )
            ...
            self.logger.debug(
                f"Target values: {[round(float(v), 3) for v in target_values]}"
            )
            ...
            for batch in range(self.batch_size):
                ...
                with torch.no_grad():
                    ...
                    y_true = [
                        ...
                        torch.FloatTensor(actions[agent][batch]),
                    ]
                    ...
                    world_map = (
                        torch.FloatTensor(states[agent][batch]["world-map"])
                        .unsqueeze(0)
                        .to(self.device)
                    )
                    flat = (
                        torch.FloatTensor(states[agent][batch]["flat"])
                        .unsqueeze(0)
                        .to(self.device)
                    )
