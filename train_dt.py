...
from trainer.policies.decision_tree import EpsGreedyLeaf, PythonDT, RandomlyInitializedEpsGreedyLeaf
from trainer.policies.grammatical_evolution import GrammaticalEvolutionTranslator, grammatical_evolution, differential_evolution
...
parser.add_argument("--n_actions", default=50, type=int, help="The number of action that the agent can perform in the environment")
...
parser.add_argument("--input_space", default=1, type=int, help="Number of inputs given to the agent")
parser.add_argument("--episodes", default=500, type=int, help="Number of episodes that the agent faces in the fitness evaluation phase")
parser.add_argument("--episode_len", default=1000, type=int, help="The max length of an episode in timesteps")
parser.add_argument("--lambda_", default=1000, type=int, help="Population size")
...
class CLeaf(RandomlyInitializedEpsGreedyLeaf):
    def __init__(self, n_actions: int = args.n_actions):
        super(CLeaf, self).__init__(n_actions, lr, args.df, args.eps, low=args.low, up=args.up)
...
def evaluate_fitness(fitness_function, leaf, genotype, episodes=args.episodes):
    ...
    bt_p = PythonDT(phenotype, CLeaf, True)
    return fitness_function((bt, bt_p), episodes)
    ...
    for iteration in range(episodes):
        ...
        for t in range(args.episode_len):
            ...
            for k_agent, _obs in obs.items():
                if k_agent == "p":
                    action[k_agent] = planner(_obs.get('flat'))
                    if action[k_agent] is None:
                        action[k_agent] = [0 for _ in range(7)]
                else:
                    action[k_agent] = agent(_obs.get('flat'))
                    if action[k_agent] is None:
                        action[k_agent] = 0
            # previous = obs
            if any([a is None for a in action.values()]):
                break
...
if __name__ == '__main__':
    ...
    with parallel_backend("multiprocessing"):
        pop, log, hof, best_leaves = grammatical_evolution(fit_fcn, individuals=args.lambda_, generations=args.generations, jobs=args.jobs, cx_prob=args.cxp, m_prob=args.mp, logfile=logfile, seed=args.seed, mutation=args.mutation, crossover=args.crossover, initial_len=args.genotype_len, selection=args.selection)
