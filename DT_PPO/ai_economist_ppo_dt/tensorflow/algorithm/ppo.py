...
class PPO():
    def __init__(self, env: BaseEnvironment, action_space: int, seed: int = None, batch_size: int = 32, log_level: int = logging.INFO, log_path: str = None) -> None:
        ...
        self.agent = LSTMModel(initial_state['0'], name="AgentLSTM", output_size=self.action_space.get('a'), log_level=log_level, log_path=log_path)
        ...
        self.planner = LSTMModel(initial_state['p'], name="PlannerLSTM", output_size=self.action_space.get('p'), log_level=log_level, log_path=log_path)
        # This row is added to start the @tf.function decorator and compile the function
    ...
    # @time_it
    def _act(self, state: dict, agent: str = 'a', hidden_states: dict = None) -> np.ndarray:
        """
        ...
        # Log
        self.logger.debug(f"Prediction (rounded): {[round(v, 3) for v in squeezed_prediction]}")

        
        if hidden_states is None:
            hidden_states = {'state_in_h_p': None, 'state_in_c_p': None, 'state_in_h_v': None, 'state_in_c_v': None}
        ...
        if agent == 'a':
            action = random.choices(np.arange(self.action_space[agent]), weights=squeezed_prediction, k=self.action_dim[agent])
        else:
            action = []
            ...
            for i in range(self.action_dim[agent]):
                _weights = squeezed_prediction[_actions_single_dim*i:_actions_single_dim*(i+1)]
                action.append(random.choices(_possible_actions, weights=_weights, k=1)[0])
        # Log
    ...
    # @time_it
    def get_actions(self, states: np.ndarray, additional_states: dict = None) -> np.ndarray:
        """
        ...
        actions, actions_one_hot, predictions, new_input_state, values, hidden_states = {}, {}, {}, {}, {}, {}

        # Iterate over agents
        for agent in states.keys():
            
            # Get the action, one-hot encoded action, and prediction
            action, one_hot_action, prediction, n_i_s, v, h_s = self._act(states[agent], 'a' if agent != 'p' else 'p', additional_states)
            # Log
        ...
        return actions, actions_one_hot, predictions, new_input_state, values, hidden_states

    def train(self, states: dict, actions: dict, rewards: dict, predictions: dict, next_states: dict, values: dict, hidden_states: dict) -> dict:
        """
        ...
        """
        losses = {'0': {'actor': [], 'critic': []}, '1': {'actor': [], 'critic': []}, '2': {'actor': [], 'critic': []}, '3': {'actor': [], 'critic': []}, 'p': {'actor': [], 'critic': []}}

        self.logger.warning("For now removing the 'p' agent from the training. In future THIS MUST BE FIXED.")

        hidden_states = {
            'a': {'state_in_h_p': [], 'state_in_c_p': [], 'state_in_h_v': [], 'state_in_c_v': []},
            'p': {'state_in_h_p': [], 'state_in_c_p': [], 'state_in_h_v': [], 'state_in_c_v': []},
        }
        ...
        for agent in states.keys():
            ...
            pred_values = [tf.convert_to_tensor(0.0)] + [tf.squeeze(v) for v in values[agent][:-1]]
            actual_values = [tf.squeeze(v) for v in values[agent]]
            gaes, target_values = self._get_gaes(rewards[agent],pred_values,actual_values)
            # Log
    ...
    def _convert_states_to_tensors(self, states: list) -> list:
        ...
        for agent, state in states.items():
            if agent != 'p':
                states[agent] = self.agent.prepare_inputs(state, append_hidden_states=False)
            else:
                states[agent] = self.planner.prepare_inputs(state, append_hidden_states=False)
    ...
    def populate_batch(self, agents: list = ['0', '1', '2', '3', 'p']) -> dict:
        ...
        for iteration in range(self.batch_size):
            ...
            actions, _, predictions, new_input_state, values, h_s = self.get_actions(state, hidden_states_dict[agent][-1] if iteration > 0 else None)
            # Log
            ...
            for agent in agents:
                ...
                actions_dict[agent].append(K.expand_dims(K.expand_dims(actions[agent], axis=0), axis=0))
        ...
        return states_dict, actions_dict, rewards_dict, predictions_dict, next_states_dict, values_dict, hidden_states_dict
