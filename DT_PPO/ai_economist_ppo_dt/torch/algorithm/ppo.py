...
class PPO:
    def __init__(
        ...
        if buffer_size > 1000:
            self.logger.warning(
                f"Batch size is very large: {buffer_size}. This may cause memory issues (in particular exponential storage time)."
            )
        ...
        self.actor = LinearModel(
            initial_state["0"]["flat"].size,
            action_space=self.action_space.get("a"),
            num_fc=0,
        ).to(self.device)
        self.planner = LinearModel(
            initial_state["p"]["flat"].size,
            action_space=self.action_space.get("p"),
            num_fc=0,
        ).to(self.device)
    ...
    def _get_gaes(
        ...
        next_values: List[torch.FloatTensor],
        gamma: float = 0.998,
        lamda: float = 0.98,
        normalize: bool = True,
    ) -> torch.FloatTensor:
        ...
        for i in range(len(rewards)):
            self.logger.debug(
                f"\t{round(rewards[i].item(),3)} + {gamma} * {round(next_values[i].item(),3)} ({round(gamma*next_values[i].item(),3)}) - {round(values[i].item(), 2)} = {round((rewards[i] + gamma * next_values[i] - values[i]).item(),3)}"
            )
        ...
        for i in reversed(range(len(deltas) - 1)):
            self.logger.debug(
                f"\t[{i}]{round(gaes[i].item(),3)} + {gamma} * {lamda} * [{i+1}]{round(gaes[i + 1].item(),3)} = {round((gaes[i] + gamma * lamda * gaes[i + 1]).item(),3)}"
            )
    ...
    def get_actions(self, states: Dict[str, list]) -> np.ndarray:
        ...
        for agent in states.keys():
            ...
            action, prediction, value = self._act(
                states[agent], "a" if agent != "p" else "p"
            )
            # Log
            if agent == "p":
                self.logger.debug(
                    f"Agent '{agent}' action: {[p.item() for p in action]}"
                )
            else:
        ...
        for agent in state.keys():
            ...
            for key, value in state[agent].items():
                if key in ["flat", "time", "p0", "p1", "p2", "p3"]:
                    temp_state[key] = (
                        torch.FloatTensor(value).unsqueeze(0).to(self.device)
                    )
                else:
                    temp_state[key] = (
                        torch.IntTensor(value).unsqueeze(0).to(self.device)
                    )
    ...
    def populate_batch(
        ...
        for iteration in range(self.buffer_size):
            ...
            # Log
            _debug_actions = {
                agent: action.item() if agent != "p" else [a.item() for a in action]
            ...
            step_actions = {
                agent: action.detach().numpy() for agent, action in actions.items()
            }
            next_state, rewards, _, _ = self.env.step(step_actions)
            next_state = self.convert_state_to_tensor(next_state)
            # Log
            _debug_rewards = {
                agent: round(reward, 3) for agent, reward in rewards.items()
            }
            ...
            for agent in agents:
                ...
                rewards_dict[agent].append(
                    torch.FloatTensor([rewards[agent]]).unsqueeze(0).to(self.device)
                )
        ...
        self.logger.info(
            f"Buffer of size {self.buffer_size} created in {ms_to_time((time.time() - start_timer)*1000)}. [mm:]ss.ms"
        )
        ...
        self.logger.info(
            f"Sum of rewards for the batch: {np.sum([np.sum(rewards_dict[agent]) for agent in agents])}"
        )
    ...
    def train(
        ...
        losses = {
            ...
            "2": [],
            "3": [],
            "p": {"Total": np.inf, "Value": np.inf, "Action": np.inf},
        }

        self.logger.info(
            f"Training {len(states.keys())} agents for {self.epochs} epochs."
        )
        ...
        for agent in states.keys():
            if agent != "p":
                ...
                self.logger.debug(
                    f"Next values: {[round(_v.item(),3) for _v in _next_values]}"
                )

                # Calculate GAEs and target values
                gaes, target_values = self._get_gaes(
                    rewards[agent], _values, _next_values
                )
                # Log
                self.logger.debug(f"GAEs: {[round(v.item(), 3) for v in gaes]}")
                self.logger.debug(
                    f"Target values: {[round(v.item(), 3) for v in target_values]}"
                )
        ...
        self.logger.info(
            f"Training took {round(time.time() - start_timer, 2)} seconds."
        )
        for key, value in losses.items():
            self.logger.info(
                f"Loss agent {key}: {round(value['Total'], 3)}, Critic: {round(value['Value'], 3)}, Actor: {round(value['Action'], 3)}"
            )
